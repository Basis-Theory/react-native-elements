...
export const JWE = (() => {
   ...
   */
  const _deriveKey = (sharedSecret: Uint8Array, algorithm: string): Uint8Array => {
    const keyLength = 32;
  ...
  const createJWE = async (
    ...
    try {
      ...
      const publicKeyBytes = decodePublicKey(recipientPublicKey);
      const { ephemeralPublicKey, sharedSecret } =
        await _performECDH(publicKeyBytes);
      ...
      const encryptedWithTag = _encryptAESGCM(
        payloadBytes,
        derivedKey,
        iv,
        additionalData
